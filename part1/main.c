...
void cleanup() {
    sem_unlink("/sem_patients");
    sem_unlink("/sem_duty_doctors");
    sem_unlink("/sem_doctors");
    sem_close(sem_patients);
    sem_close(sem_duty_doctors);
    sem_close(sem_doctors);
...
int main() {
    ...
    // Создаем именованные семафоры для синхронизации доступа к разделяемой памяти
    sem_patients = sem_open("/sem_patients", O_CREAT, 0666, 0);
    sem_duty_doctors = sem_open("/sem_duty_doctors", O_CREAT, 0666, NUM_DUTY_DOCTOR);
    sem_doctors = sem_open("/sem_doctors", O_CREAT, 0666, NUM_DOCTORS);
